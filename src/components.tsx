...
const ROWS_ALPHA = ['qwertyuiop', 'asdfghjkl', 'zxcvbnm'];
const ROWS_NUM = ['1234567890', '-/:;()$&@', '.,?!#%*_+='];
...
export function Keyboard({
  ...
  const rows = num ? ROWS_NUM : ROWS_ALPHA;

  const press = (ch: string): void => {
    onChange(value + (num ? ch : upper ? ch.toUpperCase() : ch));
    if (upper && !num) setUpper(false);
  };

  return (
    <div className="w-full shrink-0 border-t border-white/10 bg-zinc-950 px-2 pt-2 pb-3">
      <div className="mb-2 flex h-16 items-center gap-2 rounded-xl bg-white/8 px-4">
        <span className="min-w-0 flex-1 truncate text-2xl">{password ? '•'.repeat(value.length) : value}</span>
        {value ? (
          <button
            type="button"
            aria-label="Clear"
            onClick={() => onChange('')}
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full text-white/60 active:bg-white/15"
          >
            <Icon name="x" size={26} />
          </button>
        ) : null}
        ...
        <div key={row} className="mb-1.5 flex justify-center gap-1.5">
          {[...row].map(ch => (
            <button
              ...
              type="button"
              onClick={() => press(ch)}
              className="h-16 min-w-14 flex-1 rounded-lg bg-white/10 text-2xl font-medium text-white active:bg-white/25"
            >
              {num ? ch : upper ? ch.toUpperCase() : ch}
            </button>
          ))}
        </div>
      ))}
      <div className="flex justify-center gap-1.5">
        {!num ? (
          <button
            ...
            className={`h-16 w-20 rounded-lg text-2xl ${upper ? 'bg-amber-400 text-black' : 'bg-white/10'} active:bg-white/25`}
          >
            ⇧
          </button>
        ) : null}
        <button
          type="button"
          onClick={() => setNum(n => !n)}
          className="h-16 w-24 rounded-lg bg-white/10 text-2xl active:bg-white/25"
        >
          {num ? 'ABC' : '123'}
        </button>
        <button
          type="button"
          onClick={() => press(' ')}
          className="h-16 flex-1 rounded-lg bg-white/10 text-2xl active:bg-white/25"
        >
          space
        </button>
        <button
          ...
          className="h-16 w-24 rounded-lg bg-white/10 text-3xl active:bg-white/25"
        >
          ⌫
        </button>
        <button
          type="button"
          onClick={onDone}
          className="h-16 w-28 rounded-lg bg-amber-400 text-2xl font-semibold text-black active:bg-amber-300"
        >
          Done
        </button>
      </div>
